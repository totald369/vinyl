#!/usr/bin/env python3
"""
전라남도 영암군 종량제봉투 판매처 xlsx → stores.jeonnam-yeongam-trash.json

입력: 읍면, 상호명, 판매소위치(주소)

  python3 scripts/import_jeonnam_yeongam_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 판매처\(영암군\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonnam-yeongam-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonnam-yeongam-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매처(영암군).xlsx"
REF_DATE = date.today().isoformat()
CACHE_VERSION = "v1-yeongam"
...
def in_yeongam_bbox(lat: float, lng: float) -> bool:
    return 34.68 <= lat <= 34.98 and 126.30 <= lng <= 126.88


def yeongam_in_text(blob: str) -> bool:
    return "영암" in (blob or "").replace(" ", "")
...
def geocode_query_variants(addr_full: str, name: str) -> list[str]:
    ...
    if is_likely_jibeon(tail):
        ...
        if road_only and road_only != tail:
            push(f"전남 영암군 {road_only}")

    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if m:
        push(f"전남 영암군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_yeongam_bbox(lat, lng):
        return None
    if not yeongam_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_full: str, key: str, display: str) -> GeoHit | None:
    tail = yeongam_tail(normalize_addr(addr_full))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if not m:
        return None
    area = collapse(m.group(1))
    for q in (f"전남 영암군 {area}", f"전라남도 영암군 {area}"):
        for d in kakao_get(GEOCODE_URL, q, key):
            hit = parse_address_doc(d, key)
            if hit:
                hit.road = display if not is_likely_jibeon(yeongam_tail(display)) else hit.road
...
def build_addr(myeon: str, addr: str) -> str:
    myeon = collapse(myeon)
    addr = collapse(addr)
    if not addr:
        return ""
    if myeon and not addr.replace(" ", "").startswith(myeon.replace(" ", "")):
        addr = f"{myeon} {addr}"
    return addr
...
def iter_rows(path: Path) -> list[tuple[str, str]]:
    ...
    for r in range(3, ws.max_row + 1):
        ...
        myeon = collapse(str(ws.cell(r, 2).value or ""))
        name = collapse(str(ws.cell(r, 4).value or ""))
        addr = collapse(str(ws.cell(r, 5).value or ""))
        ...
        try:
            ...
            continue
        out.append((name, build_addr(myeon, addr)))
    return out
...
def main() -> None:
    ...
    for name, addr_raw in iter_rows(inp):
        ...
        if not display_road or "영암" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"jeonnam-yeongam-trash-{rid}",
                "name": name,
