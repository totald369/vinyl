#!/usr/bin/env python3
"""
경기도 수원시 불연성마대 판매소 xlsx → stores.gyeonggi-suwon-special.json

시트 `Sheet1` (헤더 2행, 데이터 3행~):
  판매처명칭(A) | 주소(B) | 연락처(C) | 판매여부(D) | 규격(E)

매핑:
  - D열 'O'/'○' -> hasSpecialBag: true
  - hasTrashBag: false (불연성마대 전용)

사용:
  cd frontend
  python3 scripts/import_gyeonggi_suwon_special_from_xlsx.py \\
    --input ~/Downloads/불연성\\ 마대\\ 판매처\\ 정보\\(수원시\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeonggi-suwon-special.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeonggi-suwon-special.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "불연성 마대 판매처 정보(수원시).xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 3
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-suwon-special"
...
def gu_hint(name: str, addr_raw: str) -> str:
    for gu in SUWON_GUS:
        if gu in name or gu in addr_raw:
            return gu
    return ""
...
def format_display_addr(raw: str) -> str:
    ...
    if not a:
        return ""
    if a.startswith("경기도 수원시"):
        return a
    if a.startswith("경기 수원시"):
        return "경기도 수원시 " + a[7:].strip()
    if a.startswith("수원시"):
        return f"경기도 {a}"
    return f"경기도 수원시 {a}"
...
def normalize_addr(addr_raw: str, gu: str = "") -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    if not a.startswith("경") and not a.startswith("수원"):
        if gu:
            a = f"수원시 {gu} {a}"
        else:
            a = f"수원시 {a}"
...
def display_road_from_raw(addr_raw: str) -> str:
    return format_display_addr(collapse(addr_raw))


def suwon_tail(full: str) -> str:
    for prefix in (
        "경기도 수원시 ",
        "경기 수원시 ",
        "수원시 ",
...
def in_suwon_bbox(lat: float, lng: float) -> bool:
    return 37.23 <= lat <= 37.36 and 126.94 <= lng <= 127.12


def suwon_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("동해시", "나주시", "청주")):
        if "수원시" not in t:
            return False
    return "수원시" in t or ("경기" in t and "수원" in t)


def geocode_query_variants(addr_raw: str, name: str, gu: str) -> list[str]:
    clean_name = re.sub(r"\([^)]*\)", "", name).strip()
    norm = normalize_addr(addr_raw, gu)
    tail = suwon_tail(norm)
    out: list[str] = []
    seen: set[str] = set()
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_suwon_bbox(lat, lng):
        return None
    if not suwon_in_text(_doc_blob(d)):
        return None
...
def area_fallback(gu: str, key: str, display_road: str) -> GeoHit | None:
    if not gu:
        return None
    for q in (f"경기도 수원시 {gu}", f"경기 수원시 {gu}"):
...
def resolve_geocode(addr_raw: str, name: str, key: str, gu: str, display_road: str) -> GeoHit | None:
    has_road = bool(re.search(r"(?:로|길|대로)\s*\d", display_road))
    for q in geocode_query_variants(addr_raw, name, gu):
        if has_road:
            for d in kakao_get(GEOCODE_URL, q, key):
                hit = parse_address_doc(d, key)
                if hit:
        ...
        if not has_road:
            for d in kakao_get(GEOCODE_URL, q, key):
                ...
                if hit:
                    return hit
    return area_fallback(gu, key, display_road)
...
def main() -> None:
    ...
    for row in rows:
        ...
        if hit is None and allow:
            hit = resolve_geocode(row.addr_raw, row.name, key, row.gu, display_road)
            if hit is None:
                misses += 1
        ...
        store: dict = {
            "id": f"gyeonggi-suwon-special-{rid}",
            "name": row.name,
