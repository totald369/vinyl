#!/usr/bin/env python3
"""
전라남도 목포시 종량제봉투·불연성마대 판매소 xlsx → stores.jeonnam-mokpo-trash.json

입력 워크북 시트 2종:
  '종량제봉투 판매처'  : 연번(A) | 상호(B) | 주소(C) | 불연성마대(X/O)(D)
  '불연성마대 판매처'  : 연번(A) | 상호(B) | 주소(C)

매핑:
  - 종량제 시트 등재           -> hasTrashBag
  - 종량제 시트 D열 O/○/Y      -> hasSpecialBag
  - 불연성마대 시트 등재       -> hasSpecialBag
  - 상호명+주소 기준 병합

사용:
  cd frontend
  python3 scripts/import_jeonnam_mokpo_trash_from_xlsx.py \\
    --input ~/Downloads/목포시_종량제봉투\\ 및\\ 불연성마대\\ 판매처\\ 현황.xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonnam-mokpo-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonnam-mokpo-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "목포시_종량제봉투 및 불연성마대 판매처 현황.xlsx"
SHEET_TRASH = "종량제봉투 판매처"
SHEET_SPECIAL = "불연성마대 판매처"
DATA_START_ROW = 4
REF_DATE = "2026-01-01"
CACHE_VERSION = "v1-mokpo"
...
def in_mokpo_bbox(lat: float, lng: float) -> bool:
    return 34.68 <= lat <= 35.05 and 126.22 <= lng <= 126.62


def mokpo_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "목포시" in t:
        return True
    return ("전라남도" in t or "전남" in t) and "목포" in t
...
def normalize_mokpo_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    a = re.sub(r"^전남\s+", "전라남도 ", a)
    if not a.startswith("전라") and not a.startswith("전남") and not a.startswith("목포"):
        a = f"목포시 {a}"
    ...
    if is_likely_jibeon(tail):
        ...
        for base in road_format_variants(target):
            ...
            if road_only and road_only != t:
                push(f"전남 목포시 {road_only}")

    m = re.match(r"^(.+?(?:동|읍|면|가))", tail.replace(" ", ""))
    if m:
        push(f"전남 목포시 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_mokpo_bbox(lat, lng):
        return None
    if not mokpo_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = mokpo_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면|가))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        return None
    for q in (f"전남 목포시 {area}", f"전라남도 목포시 {area}"):
...
def _read_sheet(wb, sheet_name: str, with_special_col: bool) -> list[tuple[str, str, bool]]:
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        if not addr:
            continue
        special = flag_y(ws.cell(r, COL_SPECIAL).value) if with_special_col else False
        rows.append((name, addr, special))
    return rows


def iter_rows(path: Path) -> list[MokpoRow]:
    wb = load_workbook(path, data_only=True)
    trash_rows = _read_sheet(wb, SHEET_TRASH, with_special_col=True)
    special_rows = _read_sheet(wb, SHEET_SPECIAL, with_special_col=False)
    wb.close()

    merged: dict[str, MokpoRow] = {}

    def key_for(name: str, addr_raw: str) -> str:
        norm = normalize_mokpo_addr(addr_raw)
        road = geocode_target(norm)
        return f"{name}|{road}"

    for name, addr, special in trash_rows:
        k = key_for(name, addr)
        if k not in merged:
            merged[k] = MokpoRow(name, addr, False, False)
        merged[k].has_trash = True
        if special:
            merged[k].has_special = True

    for name, addr, _ in special_rows:
        k = key_for(name, addr)
        if k not in merged:
            merged[k] = MokpoRow(name, addr, False, False)
        merged[k].has_special = True
...
def main() -> None:
    ...
    for row in rows:
        raw = collapse(row.addr_raw)
        # 목포시 자료에 무안군 주소가 섞인 경우(남악·일로 등) 제외
        if raw.startswith("무안군") or re.search(r"(?:^|\s)무안군\s", raw):
            continue
        full_norm = normalize_mokpo_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "목포" not in display_road.replace(" ", ""):
            continue
