#!/usr/bin/env python3
"""
부산광역시 해운대구 불연성마대 판매소 xlsx → stores.busan-haeundae-special.json

시트 `Sheet1` (헤더 3행, 데이터 4행~):
  연번(A) | 판매소명칭(B) | 주소(C) | 연락처(D) | 불연성마대(E) | 기타(F)

매핑:
  - E열 'O'/'○' -> hasSpecialBag: true
  - hasTrashBag: false
  - 기존 stores.busan-haeundae-trash.json 좌표 재사용

사용:
  cd frontend
  python3 scripts/import_busan_haeundae_special_from_xlsx.py \\
    --input ~/Downloads/정보공개답변\\(불연성마대\\ 판매소-해운대구\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.busan-haeundae-special.json"
TRASH_JSON = FRONTEND / "public" / "data" / "stores.busan-haeundae-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-busan-haeundae-special.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "정보공개답변(불연성마대 판매소-해운대구).xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 4
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-busan-haeundae-special"
...
def collapse(s: str) -> str:
    return WS_RE.sub(" ", (str(s or "")).replace("\xa0", " ").replace("\u200b", "")).strip()
...
def norm_phone(raw: str) -> str | None:
    t = collapse(raw)
    if not t or t in ("000-000-0000", "0000000000"):
        return None
    digits = re.sub(r"\D", "", t)
    if digits.startswith("051") and len(digits) == 11:
        return f"051-{digits[3:6]}-{digits[6:]}"
    if digits.startswith("051") and len(digits) == 10:
        return f"051-{digits[3:6]}-{digits[6:]}"
    if digits.startswith("052") and len(digits) == 10:
...
def normalize_addr(addr_raw: str) -> str:
    ...
    a = re.sub(r"(로)(\d+)(번길)", r"\1 \2\3", a)
    a = re.sub(r"(로)(\d번길)", r"\1 \2", a)
...
def lookup_key(name: str, addr_raw: str) -> str:
    return f"{collapse(name)}|{normalize_addr(addr_raw)}"


def haeundae_tail(full: str) -> str:
    for prefix in (
        "부산광역시 해운대구 ",
        "부산광역시 ",
        "부산 해운대구 ",
...
def in_haeundae_bbox(lat: float, lng: float) -> bool:
    return 35.12 <= lat <= 35.24 and 129.08 <= lng <= 129.22


def haeundae_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    return "해운대구" in t or (("부산" in t or "부산광역시" in t) and "해운대" in t)


def geocode_query_variants(addr_raw: str, name: str) -> list[str]:
    norm = normalize_addr(addr_raw)
    tail = haeundae_tail(norm)
    ...
    if core and core != tail:
        ...
        push(f"부산광역시 해운대구 {core}")
    push(f"부산 해운대구 {tail}")
    push(f"부산광역시 해운대구 {tail}")
    ...
    if road_only and road_only != tail:
        push(f"부산광역시 해운대구 {road_only}")
    push(f"{name} 해운대구")
    push(f"{name} 부산 해운대")
    if clean_name and clean_name != name:
        push(f"{clean_name} 해운대구")
    return out
...
def load_trash_seed() -> dict[str, GeoHit]:
    ...
    for row in rows:
        ...
        road = str(row.get("roadAddress") or row.get("address") or "")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_haeundae_bbox(lat, lng):
        return None
    if not haeundae_in_text(_doc_blob(d)):
        return None
...
def iter_rows(path: Path) -> list[HaeundaeRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, HaeundaeRow] = {}
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        if not name or not addr_raw:
            continue
        if "해운대" not in addr_raw.replace(" ", ""):
            continue
...
def main() -> None:
    ...
    for row in rows:
        ...
        store: dict = {
            "id": f"busan-haeundae-special-{rid}",
            "name": row.name,
