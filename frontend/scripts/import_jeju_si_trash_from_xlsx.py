#!/usr/bin/env python3
"""
제주특별자치도 제주시 종량제봉투·불연성마대 판매소 xlsx → stores.jeju-si-trash.json

시트 `제주시 종량제봉투 판매업소` (2행~): 구분 | 판매소명 | 도로명 주소
시트 `제주시 마대 판매업소` (2행~): 구분 | 상호명 | 주소 (불연성마대)

  python3 scripts/import_jeju_si_trash_from_xlsx.py \\
    --input ~/Downloads/제주시\\ 종량제봉투，\\ 마대\\ 판매소.xlsx
  python3 scripts/import_jeju_si_trash_from_xlsx.py --refresh
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeju-si-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeju-si-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "제주시 종량제봉투， 마대 판매소.xlsx"
SHEET_TRASH = "제주시 종량제봉투 판매업소"
SHEET_SPECIAL = "제주시 마대 판매업소"
DATA_START_ROW = 2
REF_DATE = "2026-06-05"
CACHE_VERSION = "v1-jeju-si"
...
def in_jeju_si_bbox(lat: float, lng: float) -> bool:
    # 제주 본岛(애월·한경·구좌 포함)
    if 33.28 <= lat <= 33.58 and 126.15 <= lng <= 126.92:
        return True
    # 우도
    if 33.48 <= lat <= 33.53 and 126.93 <= lng <= 126.98:
        return True
    # 추자도
    if 33.88 <= lat <= 34.05 and 126.22 <= lng <= 126.42:
        return True
    return False


def jeju_si_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if "서귀포" in t:
        return False
    if "제주시" in t:
        return True
    return "제주특별자치도" in t and "제주" in t


def format_display_addr(raw: str) -> str:
    a = collapse(raw)
    a = re.sub(r"^제주\s+", "제주특별자치도 ", a)
    if not a:
        return ""
    if a.startswith("제주특별자치도"):
        return a
    if a.startswith("제주시"):
        return f"제주특별자치도 {a}"
    return f"제주특별자치도 제주시 {a}"
...
def fix_ri_road_order(a: str) -> str:
    """`세화리 구좌로 68` → `구좌읍 구좌로 68` 등 역순 지번+도로명 보정."""
    m = re.match(r"^([가-힣]+리)\s+([가-힣]+(?:로|길|대로).*)$", a)
    if not m:
        return a
    ri, road = m.group(1), m.group(2)
    ri_to_eup: dict[str, str] = {
        "세화리": "구좌읍",
        "월정리": "구좌읍",
        "김녕리": "구좌읍",
        "하도리": "구좌읍",
    ...
    eup = ri_to_eup.get(ri)
    return f"{eup} {road}" if eup else a


def normalize_jeju_si_addr(addr: str) -> str:
    core, hint = extract_paren_hint(addr)
    a = dedupe_addr(core)
    a = fix_ri_road_order(a)
    a = re.sub(r"(\d+)\s+(\d+)\s*$", r"\1-\2", a)
    a = re.sub(r"([가-힣]+(?:리|동))(\d+)", r"\1 \2", a)
    a = re.sub(r"([가-힣]+리)\s+(\d+길)", r"\1\2", a)
    if a.startswith("제주특별자치도 "):
        a = a[len("제주특별자치도 ") :]
    if a.startswith("제주시 "):
        a = a[len("제주시 ") :]
...
def jeju_si_tail(addr: str) -> str:
    norm = normalize_jeju_si_addr(addr)
    for prefix in (
        "제주특별자치도 제주시 ",
        "제주특별자치도제주시",
        "제주특별자치도 ",
    ...
    if norm.startswith("제주특별자치도"):
        return collapse(norm.replace("제주특별자치도", "", 1))
    return norm
...
def geocode_query_variants(addr_raw: str, name: str) -> list[str]:
    tail = jeju_si_tail(addr_raw)
    norm = normalize_jeju_si_addr(addr_raw)
    out: list[str] = []
    seen: set[str] = set()
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_jeju_si_bbox(lat, lng):
        return None
    if not jeju_si_in_text(_doc_blob(d)):
        return None
...
def extract_dong_area(tail: str) -> str | None:
    ...
    m2 = re.match(r"^(.+?동)\d", tail.replace(" ", ""))
    if m2:
        return collapse(m2.group(1))
    return None
...
def dong_fallback(addr_raw: str, key: str) -> GeoHit | None:
    ...
    if not area:
        _, hint = extract_paren_hint(addr_raw)
        area = hint or None
    if not area:
        return None
    orig = normalize_jeju_si_addr(addr_raw)
    for q in (f"제주시 {area}", f"제주특별자치도 제주시 {area}"):
        for d in kakao_get(GEOCODE_URL, q, key):
            hit = parse_address_doc(d, key)
            if hit:
                hit.jibeon = orig
...
def road_strip_fallback(addr_raw: str, key: str) -> GeoHit | None:
    ...
    for road_only in candidates:
        if not road_only or road_only == tail:
            continue
        for q in (f"제주시 {road_only}", f"제주특별자치도 제주시 {road_only}"):
            for d in kakao_get(GEOCODE_URL, q, key):
                hit = parse_address_doc(d, key)
                if hit:
                    hit.jibeon = orig
...
def iter_merged_rows(path: Path) -> list[JejuSiRow]:
    wb = load_workbook(path, read_only=True, data_only=True)
    merged: dict[str, JejuSiRow] = {}
    for sheet_name, set_trash, set_special in (
        ...
        for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
            ...
            if not name or not addr_raw or name in ("판매소명", "상호명", "판매소명 "):
                continue
            norm = normalize_jeju_si_addr(addr_raw)
            key = f"{name}|{norm}"
            if key in merged:
...
def main() -> None:
    ...
    for row in rows:
        if not row.has_trash and not row.has_special:
            continue
        norm_addr = normalize_jeju_si_addr(row.addr_raw)
