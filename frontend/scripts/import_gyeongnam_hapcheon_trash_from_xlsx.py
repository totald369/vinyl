#!/usr/bin/env python3
"""
경상남도 합천군 종량제봉투·불연성마대(PP마대) 판매소 xlsx → stores.gyeongnam-hapcheon-trash.json

시트 `판매소` (헤더 1~3행, 데이터 4행~):
  연번(A) | 상호(B) | 소재지(C) | 전화번호(D) | 일반봉투(E) | 음식물봉투(F) | PP마대(G) | 음식물칩(H)

매핑:
  - E·F열 '○' -> hasTrashBag
  - G열 '○' -> hasSpecialBag (PP마대)

사용:
  cd frontend
  python3 scripts/import_gyeongnam_hapcheon_trash_from_xlsx.py \\
    --input ~/Downloads/\\(정보공개\\)종량제봉투\\ 판매소\\ 지정\\ 현황-합천군.xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-hapcheon-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-hapcheon-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "(정보공개)종량제봉투 판매소 지정 현황-합천군.xlsx"
SHEET_NAME = "판매소"
DATA_START_ROW = 4
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-hapcheon"
...
def in_hapcheon_bbox(lat: float, lng: float) -> bool:
    return 35.40 <= lat <= 35.80 and 127.98 <= lng <= 128.38


def hapcheon_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "합천군" in t:
        return True
    return ("경남" in t or "경상남도" in t) and "합천" in t
...
def normalize_hapcheon_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    if a.startswith("합천군 "):
        a = a[len("합천군 ") :]
    if not a.startswith("경") and not a.startswith("합천"):
...
def hapcheon_tail(full: str) -> str:
    for prefix in (
        "경상남도 합천군 ",
        "경상남도 ",
        "경남 합천군 ",
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    if m:
        push(f"경남 합천군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_hapcheon_bbox(lat, lng):
        return None
    if not hapcheon_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = hapcheon_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in HAPCHEON_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"경남 합천군 {area}", f"경상남도 합천군 {area}"):
...
def iter_rows(path: Path) -> list[HapcheonRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, HapcheonRow] = {}

    def key_for(name: str, addr_raw: str) -> str:
        norm = normalize_hapcheon_addr(addr_raw)
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        has_trash = flag_y(ws.cell(r, COL_GENERAL).value) or flag_y(ws.cell(r, COL_FOOD).value)
        has_special = flag_y(ws.cell(r, COL_PP).value)
        if not has_trash and not has_special:
            continue
        k = key_for(name, addr)
        if k not in merged:
            merged[k] = HapcheonRow(name, addr, phone, False, False)
        rec = merged[k]
        if has_trash:
            rec.has_trash = True
        if has_special:
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_hapcheon_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "합천" not in display_road.replace(" ", ""):
            continue
        ...
        rec: dict = {
            "id": f"gyeongnam-hapcheon-trash-{rid}",
            "name": row.name,
