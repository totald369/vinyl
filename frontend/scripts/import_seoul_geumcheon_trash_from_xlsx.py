#!/usr/bin/env python3
"""
서울특별시 금천구 종량제봉투·특수마대 판매소 xlsx → stores.seoul-geumcheon-trash.json

시트 `폐기물관리팀(5명_133판매소)` (헤더 3행, 데이터 4행~):
  번호(A) | 회사명(B) | 주소(C)

매핑 (구분 컬럼 없음 — 파일명·시트명 기준 겸업 목록):
  - hasTrashBag: true
  - hasSpecialBag: true

사용:
  cd frontend
  python3 scripts/import_seoul_geumcheon_trash_from_xlsx.py \\
    --input ~/Downloads/금천구\\ 종량제봉투\\ 및\\ 특수마대\\ 판매소\\(3.31.\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.seoul-geumcheon-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-seoul-geumcheon-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "금천구 종량제봉투 및 특수마대 판매소(3.31.).xlsx"
DATA_START_ROW = 4
REF_DATE = "2026-03-31"
CACHE_VERSION = "v1-seoul-geumcheon-trash"
...
def ref_date_from_path(p: Path) -> str:
    if re.search(r"\(3\.31\.?\)", p.name):
        try:
            year = datetime.fromtimestamp(p.stat().st_mtime).year
        except OSError:
            year = 2026
        return f"{year}-03-31"
    try:
...
def is_geumcheon_addr(addr_raw: str) -> bool:
    t = (addr_raw or "").replace(" ", "")
    if any(x in t for x in ("안양", "광명", "구로구", "관악구")):
        if "금천구" not in t:
            return False
    return "금천구" in t or ("서울" in t and "금천" in t)


def dong_hint(addr_raw: str) -> str:
    for d in GEUMCHEON_DONGS:
        if d in addr_raw:
            return d
    return ""
...
def strip_building_tail(a: str) -> str:
    ...
    a = re.sub(r"\s+[A-Za-z가-힣0-9]+동\s+.*$", "", a)
    a = re.sub(r"\s+상가.*$", "", a)
    a = re.sub(r"\s+구내.*$", "", a)
    a = re.sub(
        r"\s+(?:대륭|벽산|에이스|케이씨씨|한국후지필름|승일|제이|가산|BYC|SK|GBC|R가산).*$",
        "",
        a,
        flags=re.I,
    )
...
def normalize_addr(addr_raw: str) -> str:
    ...
    compact = a.replace(" ", "")
    if hint and hint not in compact and ROAD_RE.search(a):
        a = f"{hint} {a}"
...
def geumcheon_tail(full: str) -> str:
    for prefix in (
        "서울특별시 금천구 ",
        "서울특별시 ",
        "서울 금천구 ",
...
def geocode_query_variants(addr_raw: str, name: str) -> list[str]:
    ...
    if clean_name and clean_name != name:
        push(f"{clean_name} 금천구")
    return out


def in_geumcheon_bbox(lat: float, lng: float) -> bool:
    return 37.43 <= lat <= 37.49 and 126.85 <= lng <= 126.94


def geumcheon_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if "금천구" in t:
        return True
    return ("서울" in t or "서울특별시" in t) and "금천" in t
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_geumcheon_bbox(lat, lng):
        return None
    if not geumcheon_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    ...
    if not hint:
        return None
    for q in (f"서울 금천구 {hint}", f"서울특별시 금천구 {hint}"):
...
def iter_rows(path: Path) -> list[GeumcheonRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb.active
    merged: dict[str, GeumcheonRow] = {}
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        if not name or not addr_raw:
            continue
        if not is_geumcheon_addr(addr_raw):
            continue
        display = display_road_from_raw(addr_raw)
        k = f"{name}|{geocode_target(addr_raw)}"
        if k not in merged:
            merged[k] = GeumcheonRow(name=name, addr_raw=addr_raw, display_road=display)
...
def main() -> None:
    ...
    for row in rows:
        ...
        out.append(
            {
                "id": f"seoul-geumcheon-trash-{rid}",
                "name": row.name,
