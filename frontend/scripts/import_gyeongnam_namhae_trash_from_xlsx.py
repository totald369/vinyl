#!/usr/bin/env python3
"""
경상남도 남해군 종량제봉투·불연성마대 판매소 xlsx → stores.gyeongnam-namhae-trash.json

시트 `쓰레기종량제봉투판매업(2026. 5월) ` (헤더 3행, 데이터 4행~):
  판매처 명칭(A) | 주소(B) | 종량제봉투(C) | 특수마대(D) | 비고(E)
...
사용:
  cd frontend
  python3 scripts/import_gyeongnam_namhae_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 판매소\\ 현황\\(남해군\\)_2026.5.\\ 기준.xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-namhae-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-namhae-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매소 현황(남해군)_2026.5. 기준.xlsx"
DATA_START_ROW = 4
REF_DATE = "2026-05-01"
CACHE_VERSION = "v1-namhae"
...
def in_namhae_bbox(lat: float, lng: float) -> bool:
    return 34.68 <= lat <= 34.96 and 127.84 <= lng <= 128.10


def namhae_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "남해군" in t:
        return True
    return ("경남" in t or "경상남도" in t) and "남해" in t
...
def normalize_namhae_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    a = re.sub(r"\s*번지\s*", " ", a)
    compact = a.replace(" ", "")
    compact = re.sub(r"((?:대로|로))(\d+번(?:가)?길)", r"\1 \2", compact)
    compact = re.sub(r"(번(?:가)?길)(\d+)", r"\1 \2", compact)
    compact = re.sub(r"((?:대로|로|길))(\d+(?:-\d+)?)", r"\1 \2", compact)
    compact = re.sub(r"([가-힣]+(?:리|동))(\d+)", r"\1 \2", compact)
    a = collapse(compact)
    if not a.startswith("경") and not a.startswith("남해"):
...
def road_format_variants(addr: str) -> list[str]:
    ...
    push(compact)
    push(re.sub(r"(\d+번(?:가)?길)(\d+)$", r"\1 \2", compact))
    push(re.sub(r"((?:로|길|대로))(\d+)", r"\1 \2", compact))
    return out


def namhae_tail(full: str) -> str:
    for prefix in (
        "경상남도 남해군 ",
        "경상남도 ",
        "경남 남해군 ",
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    if m:
        push(f"경남 남해군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_namhae_bbox(lat, lng):
        return None
    if not namhae_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = namhae_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in NAMHAE_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"경남 남해군 {area}", f"경상남도 남해군 {area}"):
...
def pick_sheet(wb) -> object:
    for name in wb.sheetnames:
        if "종량제" in name or "판매" in name:
            return wb[name]
    return wb.active


def iter_rows(path: Path) -> list[NamhaeRow]:
    wb = load_workbook(path, data_only=True)
    ws = pick_sheet(wb)
    merged: dict[str, NamhaeRow] = {}

    def key_for(name: str, addr_raw: str) -> str:
        norm = normalize_namhae_addr(addr_raw)
        road = geocode_target(norm)
        return f"{name}|{road}"

    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        name = cell_str(ws.cell(r, COL_NAME).value)
        if not name or name in ("판매처 명칭(상호명)", "판매소명"):
            continue
        ...
        k = key_for(name, addr)
        if k not in merged:
            merged[k] = NamhaeRow(name, addr, False, False)
        rec = merged[k]
        if has_trash:
            rec.has_trash = True
        if has_special:
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_namhae_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "남해" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"gyeongnam-namhae-trash-{rid}",
                "name": row.name,
