#!/usr/bin/env python3
"""
경상남도 의령군 종량제봉투·불연성마대 판매소 xlsx → stores.gyeongnam-uiryeong-trash.json

시트 `종량제판매소 위치 정보` (헤더 5행, 데이터 6행~):
  지자체명(A) | 명칭(B) | 도로명주소(C) | 종량제판매여부(D) | 마대판매여부(E) | 규격별(D~R)

매핑:
  - D열 또는 F~R열 'Y'/'O' -> hasTrashBag
  - E열 'Y'/'O' -> hasSpecialBag

사용:
  cd frontend
  python3 scripts/import_gyeongnam_uiryeong_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 판매소\\ 위치정보\\ \\(의령군\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-uiryeong-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-uiryeong-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매소 위치정보 (의령군).xlsx"
SHEET_NAME = "종량제판매소 위치 정보"
DATA_START_ROW = 6
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-uiryeong"

COL_NAME = 2
COL_ADDR = 3
COL_TRASH_FLAG = 4
COL_SPECIAL = 5
COL_TRASH_DETAIL_START = 6
COL_TRASH_DETAIL_END = 18
...
def in_uiryeong_bbox(lat: float, lng: float) -> bool:
    return 35.22 <= lat <= 35.46 and 128.10 <= lng <= 128.38


def uiryeong_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "의령군" in t:
        return True
    return ("경남" in t or "경상남도" in t) and "의령" in t
...
def normalize_uiryeong_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    if not a.startswith("경") and not a.startswith("의령"):
...
def uiryeong_tail(full: str) -> str:
    for prefix in (
        "경상남도 의령군 ",
        "경상남도 ",
        "경남 의령군 ",
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    if m:
        push(f"경남 의령군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_uiryeong_bbox(lat, lng):
        return None
    if not uiryeong_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = uiryeong_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in UIRYEONG_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"경남 의령군 {area}", f"경상남도 의령군 {area}"):
...
def row_flags(ws, r: int) -> tuple[bool, bool]:
    has_trash = flag_y(ws.cell(r, COL_TRASH_FLAG).value) or any(
        flag_y(ws.cell(r, c).value) for c in range(COL_TRASH_DETAIL_START, COL_TRASH_DETAIL_END + 1)
    )
    has_special = flag_y(ws.cell(r, COL_SPECIAL).value)
    return has_trash, has_special


def iter_rows(path: Path) -> list[UiryeongRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, UiryeongRow] = {}

    def key_for(name: str, addr_raw: str) -> str:
        norm = normalize_uiryeong_addr(addr_raw)
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        k = key_for(name, addr)
        if k not in merged:
            merged[k] = UiryeongRow(name, addr, False, False)
        rec = merged[k]
        if has_trash:
            rec.has_trash = True
        if has_special:
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_uiryeong_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "의령" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"gyeongnam-uiryeong-trash-{rid}",
                "name": row.name,
