#!/usr/bin/env python3
"""
경상남도 산청군 종량제봉투·불연성마대 판매소 xlsx → stores.gyeongnam-sancheong-trash.json

시트 `1. 종량제판매소 위치 정보` (헤더 3행, 데이터 6행~):
  지자체명(A) | 명칭(B) | 주소(C) | 연락처(D) | 종량제봉투(E) | 특수마대(F)
...
사용:
  cd frontend
  python3 scripts/import_gyeongnam_sancheong_trash_from_xlsx.py \\
    --input ~/Downloads/\\(붙임\\)\\ 지자체별\\ 종량제봉투\\ 판매장소\\ 현황\\ 조사표\\(산청군\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-sancheong-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-sancheong-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "(붙임) 지자체별 종량제봉투 판매장소 현황 조사표(산청군).xlsx"
SHEET_NAME = "1. 종량제판매소 위치 정보"
DATA_START_ROW = 6
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-sancheong"
...
def in_sancheong_bbox(lat: float, lng: float) -> bool:
    return 35.25 <= lat <= 35.52 and 127.80 <= lng <= 128.08


def sancheong_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "산청군" in t:
        return True
    return ("경남" in t or "경상남도" in t) and "산청" in t
...
def normalize_sancheong_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    if not a.startswith("경") and not a.startswith("산청"):
...
def sancheong_tail(full: str) -> str:
    for prefix in (
        "경상남도 산청군 ",
        "경상남도 ",
        "경남 산청군 ",
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    if m:
        push(f"경남 산청군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_sancheong_bbox(lat, lng):
        return None
    if not sancheong_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = sancheong_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in SANCHEONG_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"경남 산청군 {area}", f"경상남도 산청군 {area}"):
...
def iter_rows(path: Path) -> list[SancheongRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, SancheongRow] = {}

    def key_for(name: str, addr_raw: str) -> str:
        norm = normalize_sancheong_addr(addr_raw)
        road = geocode_target(norm)
        return f"{name}|{road}"

    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        gov = cell_str(ws.cell(r, COL_GOV).value)
        ...
        if not addr:
            continue
        if gov and "산청" not in gov.replace(" ", ""):
            continue
        ...
        k = key_for(name, addr)
        if k not in merged:
            merged[k] = SancheongRow(name, addr, phone, False, False)
        rec = merged[k]
        if has_trash:
            rec.has_trash = True
        if has_special:
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_sancheong_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "산청" not in display_road.replace(" ", ""):
            continue
        ...
        rec: dict = {
            "id": f"gyeongnam-sancheong-trash-{rid}",
            "name": row.name,
