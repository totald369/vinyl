#!/usr/bin/env python3
"""
서울특별시 성북구 특수규격봉투(불연성마대) 판매소 xlsx → stores.seoul-seongbuk-special.json

시트 `Sheet1` (헤더 1행, 데이터 2행~):
  연번(A) | 상호명(B) | 연락처(C) | 판매여부(D) | 시군(E) | 구명(F) | 주소(G) | 규격(H)
...
사용:
  cd frontend
  python3 scripts/import_seoul_seongbuk_special_from_xlsx.py \\
    --input ~/Downloads/성북구_불연성마대판매소.xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.seoul-seongbuk-special.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-seoul-seongbuk-special.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "성북구_불연성마대판매소.xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 2
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-seoul-seongbuk-special"
...
def norm_phone(raw: str) -> str | None:
    ...
    if len(digits) == 7 and not t.startswith("0"):
...
def normalize_road(addr_raw: str) -> str:
    ...
    a = re.sub(r"(\d+)(길)(\d+)", r"\1\2 \3", a)
    if not a.startswith("서울") and not a.startswith("성북"):
        a = f"성북구 {a}"
...
def seongbuk_tail(full: str) -> str:
    for prefix in (
        "서울특별시 성북구 ",
        "서울특별시 ",
        "서울 성북구 ",
...
def in_seongbuk_bbox(lat: float, lng: float) -> bool:
    return 37.57 <= lat <= 37.62 and 126.98 <= lng <= 127.05


def seongbuk_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if "성북구" in t:
        return True
    return ("서울" in t or "서울특별시" in t) and "성북" in t


def geocode_query_variants(addr_raw: str, name: str) -> list[str]:
    road = normalize_road(addr_raw)
    target = geocode_target(addr_raw)
    tail = seongbuk_tail(target)
    out: list[str] = []
    seen: set[str] = set()
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_seongbuk_bbox(lat, lng):
        return None
    if not seongbuk_in_text(_doc_blob(d)):
        return None
...
def iter_rows(path: Path) -> list[SeongbukRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, SeongbukRow] = {}
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        if not flag_y(ws.cell(r, COL_SPECIAL).value):
            continue
        gu = cell_str(ws.cell(r, 6).value)
        if gu and gu != "성북구":
            continue
        phone = norm_phone(cell_str(ws.cell(r, COL_PHONE).value)) or ""
        display = normalize_road(addr_raw)
        k = f"{name}|{display}"
        if k not in merged:
            merged[k] = SeongbukRow(
                name=name,
                addr_raw=addr_raw,
                phone=phone,
...
def main() -> None:
    ...
    for row in rows:
        ...
        store: dict = {
            "id": f"seoul-seongbuk-special-{rid}",
            "name": row.name,
