#!/usr/bin/env python3
"""
전북특별자치도 남원시 종량제봉투 판매소 xlsx → stores.jeonbuk-namwon-trash.json

시트 `Sheet1` (헤더 5행, 데이터 6행~):
  연번(B) | 판매소명(C) | 입력 주소(D)

모든 판매소는 종량제봉투 취급(hasTrashBag=True).
입력 주소가 동·지번만 있는 경우가 많아 키워드 검색을 적극 활용한다.

사용:
  cd frontend
  python3 scripts/import_jeonbuk_namwon_trash_from_xlsx.py \\
    --input ~/Downloads/남원시_봉투판매소\\ 현황(2026.6.9.).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonbuk-namwon-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonbuk-namwon-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "남원시_봉투판매소 현황(2026.6.9.).xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 6
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-namwon"
...
def in_namwon_bbox(lat: float, lng: float) -> bool:
    # 인월·산내(지리산) 일대 포함
    return 35.15 <= lat <= 35.58 and 127.12 <= lng <= 127.68


def namwon_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "남원시" in t:
        return True
    return ("전북" in t or "전라북도" in t) and "남원" in t
...
def normalize_namwon_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    a = re.sub(r",\s*.*$", "", a)
    a = re.sub(r"\s*번지\s*", " ", a)
    a = re.sub(r"([가-힣]+(?:동|리))(\d+)", r"\1 \2", a)
    a = re.sub(r"((?:대로|로|길))(\d+(?:-\d+)?)", r"\1 \2", a)
    if not a.startswith("전") and not a.startswith("남원"):
        a = f"남원시 {a}"
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_namwon_bbox(lat, lng):
        return None
    if not namwon_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = namwon_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면|리))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        return None
    for q in (f"전북 남원시 {area}", f"전북특별자치도 남원시 {area}"):
...
def resolve_geocode(addr_raw: str, name: str, key: str, display_road: str) -> GeoHit | None:
    queries = geocode_query_variants(display_road, name)
...
def iter_rows(path: Path) -> list[NamwonRow]:
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        if not addr:
            continue
        out.append(NamwonRow(name=name, addr_raw=addr))
    wb.close()
    return out
...
def main() -> None:
    ...
    for row in rows:
        ...
        if re.search(r"경기|고양|서울|부산|대구|인천|대전|울산|광주", raw):
            continue
        full_norm = normalize_namwon_addr(row.addr_raw)
