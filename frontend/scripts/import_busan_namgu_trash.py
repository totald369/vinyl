#!/usr/bin/env python3
"""
부산광역시 남구시설관리공단 '종량제 봉투 판매소' 게시판 크롤
  https://www.bnfmc.or.kr/reservation/www/18?page=1 .. 17
- 총 326건(20건/페이지), 모두 종량제봉투 판매처로 반영(hasTrashBag=True)
- 카카오 주소·키워드 검색 지오코딩 (캐시: geocode-cache-busan-namgu-trash.json)

필요: frontend/.env.local 의 KAKAO_REST_API_KEY (또는 KAKAO_REST_KEY)

  python3 scripts/import_busan_namgu_trash.py
  python3 scripts/import_busan_namgu_trash.py --max-pages 1   # 스모크 테스트
"""
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.busan-namgu-trash.json"
CACHE_PATH = Path(__file__).resolve().parent / "geocode-cache-busan-namgu-trash.json"

LIST_TMPL = "https://www.bnfmc.or.kr/reservation/www/18?page={page}"
...
def fetch_page(page: int) -> str:
    url = LIST_TMPL.format(page=page)
    r = urllib.request.Request(
        url,
        headers={
...
def parse_table_rows(html: str) -> list[tuple[str, str, str, str, str, str]]:
    """
    (게시번호, 동, 판매소명, 주소, 연락처, 지도링크).
    """
    m = re.search(
        r'<tbody[^>]*class="txtcenter"[^>]*>([\s\S]*?)</tbody>',
        html,
        re.IGNORECASE,
    )
    if not m:
    ...
    for tr in re.finditer(r"<tr[^>]*>([\s\S]*?)</tr>", block):
        ...
        dong = _clean_cell(tds[1])
        name = _clean_cell(tds[2])
        addr = _clean_cell(tds[3])
        phone = _clean_cell(tds[4])
        map_cell = tds[5]
        href_m = re.search(r'href="(https?://[^"]+)"', map_cell)
        map_href = href_m.group(1).strip() if href_m else ""
        if not board_num.isdigit():
            continue
        if dong == "동" or name == "판매소명":
            continue
        if not name or not addr:
            continue
        out.append((board_num, dong, name, addr, phone, map_href))
    return out


def full_busan_road(raw: str, dong: str) -> str:
    ...
    # 지번만·번지만인 경우 행정동 힌트로 접두
    if not any(x in s for x in ("부산", "남구", "로", "길", "대로")):
        d = (dong or "").replace(" ", "")
        if d.endswith("동"):
            s = f"부산광역시 남구 {d} {s}"
        else:
...
def main():
    ...
    ap.add_argument("--max-pages", type=int, default=17)
    args = ap.parse_args()
    ...
    print(f"총 {len(all_rows)}행 파싱", file=sys.stderr)
    nums = sorted({int(r[0]) for r in all_rows})
    print(
        f"  게시번호 범위: {nums[0] if nums else '—'}..{nums[-1] if nums else '—'}",
        file=sys.stderr,
    )
    ...
    if not KAKAO_REST_KEY:
        print(
            "KAKAO_REST_API_KEY 없음. 좌표 없이 종료합니다. JSON은 비워지지 않게 하려면 키를 설정하세요.",
            file=sys.stderr,
        )

    for i, (board_num, dong, name, addr_raw, phone, _map_href) in enumerate(all_rows):
        road = full_busan_road(addr_raw, dong)
        oid = f"busan-namgu-trash-{board_num}"
