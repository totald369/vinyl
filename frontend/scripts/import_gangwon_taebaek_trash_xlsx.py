#!/usr/bin/env python3
"""
강원특별자치도 태백시 종량제봉투 지정판매소 xlsx -> stores.gangwon-taebaek-trash.json

시트 '총괄', 헤더: 일련번호, 상호, 업종, 영업소재지, 비고
원본에 기준일 컬럼 없음 → dataReferenceDate 기본 2026-01-01

  python3 scripts/import_gangwon_taebaek_trash_xlsx.py \\
    --xlsx ~/Downloads/'종량제봉투 지정판매소 현황.xlsx'
"""
...
DEFAULT_XLSX = Path.home() / "Downloads" / "종량제봉투 지정판매소 현황.xlsx"
OUT_JSON = FRONTEND / "public" / "data" / "stores.gangwon-taebaek-trash.json"
CACHE_PATH = Path(__file__).resolve().parent / "geocode-cache-gangwon-taebaek-trash.json"
...
def normalize_addr(addr: str) -> str:
    ...
    if not a:
        return a
    if a.startswith("강원특별자치도 태백시"):
        return a
    if a.startswith("강원도 태백시"):
        return "강원특별자치도 태백시" + a[len("강원도 태백시") :].strip()
    if a.startswith("태백시"):
        return "강원특별자치도 " + a
    return f"강원특별자치도 태백시 {a}"
...
def in_taebaek_bbox(lat: float, lng: float) -> bool:
    return 37.06 <= lat <= 37.26 and 128.88 <= lng <= 129.10


def read_rows_from_xlsx(path: Path) -> list[dict[str, str]]:
    wb = load_workbook(path, read_only=True, data_only=True)
    if "총괄" not in wb.sheetnames:
        names = wb.sheetnames
        wb.close()
        raise SystemExit(f"시트 '총괄' 없음: {names}")
    ws = wb["총괄"]
    ...
    for tup in ws.iter_rows(values_only=True):
        ...
        if len(row) < 4:
            continue
        name_c = collapse(str(row[1] or ""))
        if name_c in ("", "상호"):
            continue
        addr_s = normalize_addr(str(row[3] or ""))
        if not addr_s:
            continue
        out.append(
            {
...
def main() -> None:
    ...
    for i, r in enumerate(rows, start=1):
        sid = f"gangwon-taebaek-trash-{i:04d}"
        ck = hashlib.sha1(
        ...
        if ck in cache:
            ...
            if lat is None or lng is None:
                continue
            if not in_taebaek_bbox(lat, lng):
                continue
            cache[ck] = [lat, lng]
            if i % 40 == 0:
                save_cache(cache)
        ...
        if not in_taebaek_bbox(float(lat), float(lng)):  # type: ignore[arg-type]
            continue
