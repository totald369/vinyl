#!/usr/bin/env python3
"""
전라남도 해남군 종량제봉투·불연성마대 판매소 xlsx → stores.jeonnam-haenam-trash.json

시트 '종량제판매소 위치 정보' (헤더 3행, 데이터 5행~):
  판매소 명칭(B) | 도로명 주소(C) | 종량제 봉투(D) | 불연성 마대(E)
...
사용:
  cd frontend
  python3 scripts/import_jeonnam_haenam_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 판매소(전남해남군).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonnam-haenam-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonnam-haenam-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매소(전남해남군).xlsx"
SHEET_NAME = "종량제판매소 위치 정보"
DATA_START_ROW = 5
REF_DATE = "2026-01-01"
CACHE_VERSION = "v1-haenam"
...
def in_haenam_bbox(lat: float, lng: float) -> bool:
    return 34.28 <= lat <= 34.78 and 126.28 <= lng <= 126.78


def haenam_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "해남군" in t:
        return True
    return ("전라남도" in t or "전남" in t) and "해남" in t
...
def normalize_haenam_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    a = re.sub(r"^전남\s+", "전라남도 ", a)
    if not a.startswith("전라") and not a.startswith("전남") and not a.startswith("해남"):
        a = f"해남군 {a}"
    ...
    if is_likely_jibeon(tail):
        ...
        for base in road_format_variants(target):
            ...
            if road_only and road_only != t:
                push(f"전남 해남군 {road_only}")

    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if m:
        push(f"전남 해남군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_haenam_bbox(lat, lng):
        return None
    if not haenam_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = haenam_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in HAENAM_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"전남 해남군 {area}", f"전라남도 해남군 {area}"):
...
def iter_rows(path: Path) -> list[HaenamRow]:
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_haenam_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "해남" not in display_road.replace(" ", ""):
            continue
