#!/usr/bin/env python3
"""
서울시 노원구 쓰레기(종량제)봉투 판매업 인허가 JSON → stores.sample.json 병합.
- 영업만: sals_stts_cd 가 '01' 인 행만
- 도로명(또는 지번) 주소로 카카오 지오코딩 (xcrd/ycrd 는 지도 좌표계로 미사용)

사용:
  .env.local 의 KAKAO_REST_API_KEY
  python3 scripts/import_nowon_json.py
  python3 scripts/import_nowon_json.py /path/to/....json
"""
...
CACHE_PATH = Path(__file__).resolve().parent / "geocode-cache-nowon.json"
...
def parse_json(path: Path) -> list[dict]:
    ...
    for row in rows_in:
        ...
        if not addr.startswith("서울"):
            addr = f"서울특별시 노원구 {addr}"
        addr_alts = [
            a if a.startswith("서울") else f"서울특별시 노원구 {a}" for a in addr_alts
        ]
        out.append(
            {
                "name": name,
...
def discover_paths(argv: list[str]) -> list[Path]:
    ...
    for p in DOWNLOADS.glob("*.json"):
        n = p.name
        if "노원" in n and ("봉투" in n or "종량제" in n or "쓰레기" in n):
...
def main():
    ...
    if not paths:
        print("JSON 경로를 인자로 주거나, Downloads 에 노원 관련 JSON 을 두세요.")
        raise SystemExit(1)
