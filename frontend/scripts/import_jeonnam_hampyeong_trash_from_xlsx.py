#!/usr/bin/env python3
"""
전라남도 함평군 종량제봉투 판매소 xlsx → stores.jeonnam-hampyeong-trash.json

시트 '1. 종량제판매소 위치 정보' (헤더 3행, 데이터 5행~):
  연번(A) | 판매소 명칭(B) | 도로명 주소(C) | 상세주소(D)

모든 판매소는 종량제봉투 취급(hasTrashBag=True).

사용:
  cd frontend
  python3 scripts/import_jeonnam_hampyeong_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 판매소\\ 위치정보\\ (함평군).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonnam-hampyeong-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonnam-hampyeong-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매소 위치정보 (함평군).xlsx"
SHEET_NAME = "1. 종량제판매소 위치 정보"
DATA_START_ROW = 5
REF_DATE = "2026-01-01"
CACHE_VERSION = "v1-hampyeong"
...
def in_hampyeong_bbox(lat: float, lng: float) -> bool:
    return 34.95 <= lat <= 35.30 and 126.30 <= lng <= 126.70


def hampyeong_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "함평군" in t:
        return True
    return ("전라남도" in t or "전남" in t) and "함평" in t


def fix_typos(a: str) -> str:
    return a.replace("전란남도", "전라남도")


def format_display_addr(raw: str) -> str:
    a = fix_typos(collapse(raw))
...
def normalize_hampyeong_addr(addr_raw: str, area: str = "") -> str:
    a = fix_typos(collapse(addr_raw))
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    a = re.sub(r",\s*.*$", "", a)
    ...
    a = re.sub(r"함평군(?=[가-힣])", "함평군 ", a)
    if not a.startswith("전라") and not a.startswith("전남") and not a.startswith("함평"):
...
def hampyeong_tail(full: str) -> str:
    for prefix in (
        "전라남도 함평군 ",
        "전라남도 ",
        "전남 함평군 ",
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_hampyeong_bbox(lat, lng):
        return None
    if not hampyeong_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = hampyeong_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in HAMPYEONG_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"전남 함평군 {area}", f"전라남도 함평군 {area}"):
...
def iter_rows(path: Path) -> list[HampyeongRow]:
    # read_only 모드는 거대한 열 차원(빈 셀 포함)에서 ws.cell() 임의접근이 매우 느려
    # 일반 모드로 연다.
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    out: list[HampyeongRow] = []
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        name = cell_str(ws.cell(r, COL_NAME).value)
        if not name or name in ("종량제봉투 판매소 명칭", "판매소 명칭"):
            continue
        ...
        if not addr:
            continue
        out.append(HampyeongRow(name=name, addr_raw=addr))
    wb.close()
    return out
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_hampyeong_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "함평" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"jeonnam-hampyeong-trash-{rid}",
                "name": row.name,
