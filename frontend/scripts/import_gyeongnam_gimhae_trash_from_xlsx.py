#!/usr/bin/env python3
"""
경상남도 김해시 종량제봉투 판매소 xlsx → stores.gyeongnam-gimhae-trash.json

시트: 연번 | 읍면동 | 판매소명 | 사업장 주소

  pip install openpyxl
  python3 scripts/import_gyeongnam_gimhae_trash_from_xlsx.py \\
    --input ~/Downloads/현황.xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-gimhae-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-gimhae-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "현황.xlsx"
REF_DATE = "2026-05-23"
CACHE_VERSION = "v1"
...
def in_gimhae_bbox(lat: float, lng: float) -> bool:
    # 삼계·삼안·구산 일대(동경 128.91) 포함
    return 35.08 <= lat <= 35.38 and 128.65 <= lng <= 128.93


def gimhae_in_text(blob: str) -> bool:
    return "김해" in (blob or "").replace(" ", "")
...
def format_display_addr(raw: str) -> str:
    ...
    if a.startswith("김해시"):
        return f"경상남도 {a}"
    if gimhae_in_text(a) or re.search(r"(동|읍|면)\s", a):
        return f"경상남도 김해시 {a}"
    return a


def normalize_addr(admin_dong: str, addr_raw: str) -> str:
    dong = collapse(admin_dong)
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    if not a.startswith("김해") and not a.startswith("경상"):
        if dong and not a.startswith(dong):
            a = f"{dong} {a}"
        a = f"김해시 {a}"
    elif dong and dong.replace(" ", "") not in a.replace(" ", ""):
        if a.startswith("김해시"):
            a = f"김해시 {dong} {a[len('김해시'):].lstrip()}"
        elif a.startswith("경상남도 김해시"):
            a = f"경상남도 김해시 {dong} {a[len('경상남도 김해시'):].lstrip()}"
...
def road_format_variants(addr: str) -> list[str]:
    ...
    push(re.sub(r"(로)(\d+번길)", r"\1\2", addr.replace(" ", "")))
    push(re.sub(r"([가-힣]+로)(\d+)", r"\1 \2", addr.replace(" ", "")))
    return out


def gimhae_tail(full: str) -> str:
    for prefix in ("경상남도 김해시 ", "경상남도 ", "경남 김해시 ", "김해시 "):
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    for base in road_format_variants(target):
        tail = gimhae_tail(base)
        push(f"경남 김해시 {tail}")
        push(f"경상남도 김해시 {tail}")
        push(base)
        ...
        if road_only and road_only != tail:
            push(f"경남 김해시 {road_only}")
        m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
        if m:
            push(f"경남 김해시 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_gimhae_bbox(lat, lng):
        return None
    if not gimhae_in_text(_doc_blob(d)):
        return None
...
def area_fallback(road_full: str, key: str, display_road: str) -> GeoHit | None:
    tail = gimhae_tail(geocode_target(road_full))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if not m:
        return None
    area = collapse(m.group(1))
    for q in (f"경남 김해시 {area}", f"경상남도 김해시 {area}"):
...
def find_header_row(ws) -> int:
    for i, row in enumerate(ws.iter_rows(min_row=1, max_row=10, values_only=True), start=1):
        if not row:
            continue
        texts = [cell_str(x) for x in row]
        if "연번" in texts and "판매소명" in "".join(texts) and "사업장" in "".join(texts):
            return i
    raise SystemExit("헤더 행(연번·판매소명·사업장 주소)을 찾지 못했습니다.")
...
def main() -> None:
    ...
    for row in ws.iter_rows(min_row=hdr + 1, values_only=True):
        ...
        if seq is None or cell_str(seq) == "":
            continue
        admin_dong = cell_str(row[1])
        name = cell_str(row[2])
        addr_raw = cell_str(row[3])
        if not name or not addr_raw:
            continue

        full_norm = normalize_addr(admin_dong, addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "김해" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"gyeongnam-gimhae-trash-{rid}",
                "name": name,
