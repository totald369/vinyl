#!/usr/bin/env python3
"""
전북 김제시 종량제봉투·불연성마대 판매소 xlsx → stores.jeonbuk-gimje-trash.json

  pip install openpyxl
  python3 scripts/import_jeonbuk_gimje_trash_from_xlsx.py \\
    --input ~/Downloads/김제시\\ 종량제봉투\\ 판매소\\ 목록.xlsx
  python3 scripts/import_jeonbuk_gimje_trash_from_xlsx.py --refresh
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonbuk-gimje-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonbuk-gimje-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "김제시 종량제봉투 판매소 목록.xlsx"
REF_DATE = "2026-05-21"
CACHE_VERSION = "v2"
...
def in_gimje_bbox(lat: float, lng: float) -> bool:
    return 35.65 <= lat <= 35.92 and 126.68 <= lng <= 127.08


def gimje_in_text(blob: str) -> bool:
    return "김제" in (blob or "").replace(" ", "")
...
def format_display_addr(raw: str) -> str:
    ...
    if not a.startswith("전북") and "김제" in a.replace(" ", ""):
        if a.startswith("김제시"):
            return f"전북특별자치도 {a}"
        return f"전북특별자치도 김제시 {a}"
    return a
...
def normalize_gimje_addr(addr: str) -> str:
    a = dedupe_addr(addr)
    a = a.replace("전북특별치도", "전북특별자치도")
    ...
    if a.startswith("김제시"):
        return f"전북특별자치도 {a}"
    if "김제시" in compact or "김제" in compact:
        return f"전북특별자치도 {a}" if not a.startswith("전북") else a
    return f"전북특별자치도 김제시 {a}"


def gimje_tail(addr: str) -> str:
    """카카오 API 쿼리용 — `전북 김제시 {tail}` 형태가 가장 잘 맞음."""
    norm = normalize_gimje_addr(addr)
    for prefix in (
        "전북특별자치도 김제시 ",
        "전북특별자치도김제시",
        "전북특별자치도 ",
...
def geocode_query_variants(addr_raw: str, name: str) -> list[str]:
    ...
    if is_likely_jibeon(tail):
        for tv in lot_query_variants(tail):
            push_q(f"전북 김제시 {tv}")
            push_q(f"전북특별자치도 김제시 {tv}")
    else:
        push_q(f"전북 김제시 {tail}")
        push_q(f"전북특별자치도 김제시 {tail}")
        if re.search(r"-\d+\s*$", tail):
            no_suffix = re.sub(r"-\d+\s*$", "", tail).strip()
            push_q(f"전북 김제시 {no_suffix}")

    norm = normalize_gimje_addr(addr_raw)
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_gimje_bbox(lat, lng):
        return None
    if not gimje_in_text(_doc_blob(d)):
        return None
...
def dong_fallback(addr_raw: str, key: str) -> GeoHit | None:
    ...
    for q in (f"전북 김제시 {area}", f"전북특별자치도 김제시 {area}"):
        for d in kakao_get(GEOCODE_URL, q, key):
            hit = parse_address_doc(d, key)
            if hit:
                orig = format_display_addr(normalize_gimje_addr(addr_raw))
                hit.jibeon = orig
...
def road_strip_fallback(addr_raw: str, key: str) -> GeoHit | None:
    ...
    for road_only in candidates:
        if not road_only or road_only == tail:
            continue
        for q in (f"전북 김제시 {road_only}", f"전북특별자치도 김제시 {road_only}"):
            for d in kakao_get(GEOCODE_URL, q, key):
                hit = parse_address_doc(d, key)
                if hit:
                    hit.jibeon = orig
...
def find_header_row(ws) -> tuple[int, dict[str, int]]:
    for i, row in enumerate(ws.iter_rows(min_row=1, max_row=5, values_only=True), start=1):
        ...
        if "연번" in texts and any("상호" in t for t in texts) and any("주소" in t for t in texts):
            col: dict[str, int] = {}
...
def main() -> None:
    ...
    for row in ws.iter_rows(min_row=hdr_row + 1, values_only=True):
        ...
        norm_addr = normalize_gimje_addr(addr_raw)
