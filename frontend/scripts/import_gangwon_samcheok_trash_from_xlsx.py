#!/usr/bin/env python3
"""
강원특별자치도 삼척시 종량제봉투·특수마대 판매소 xlsx → stores.gangwon-samcheok-trash.json

시트 `종량제봉투 판매소`: 판매소명 | 도로명 주소 | 종량제봉투(O/X) | 불연성 특수마대(O/X) — 3행부터

  pip install openpyxl
  python3 scripts/import_gangwon_samcheok_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 및\\ 특수마대\\ 판매처\\ 정보\\(삼척시\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gangwon-samcheok-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gangwon-samcheok-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 및 특수마대 판매처 정보(삼척시).xlsx"
REF_DATE = "2026-05-23"
CACHE_VERSION = "v1"
...
def in_samcheok_bbox(lat: float, lng: float) -> bool:
    # 삼계·삼안·구산 일대(동경 128.91) 포함
    return 37.10 <= lat <= 37.55 and 128.95 <= lng <= 129.45


def samcheok_in_text(blob: str) -> bool:
    return "삼척" in (blob or "").replace(" ", "")
...
def format_display_addr(raw: str) -> str:
    ...
    if a.startswith("삼척시"):
        return f"강원특별자치도 {a}"
    if samcheok_in_text(a) or re.search(r"(동|읍|면)\s", a):
        return f"강원특별자치도 삼척시 {a}"
    return a


def normalize_samcheok_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    if not a.startswith("삼척") and not a.startswith("강원"):
        a = f"삼척시 {a}"
...
def samcheok_tail(full: str) -> str:
    for prefix in ("강원특별자치도 삼척시 ", "강원특별자치도 ", "강원 삼척시 ", "삼척시 "):
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    for base in road_format_variants(target):
        tail = samcheok_tail(base)
        push(f"강원 삼척시 {tail}")
        push(f"강원특별자치도 삼척시 {tail}")
        push(base)
        ...
        if road_only and road_only != tail:
            push(f"강원 삼척시 {road_only}")
        m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
        if m:
            push(f"강원 삼척시 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_samcheok_bbox(lat, lng):
        return None
    if not samcheok_in_text(_doc_blob(d)):
        return None
...
def area_fallback(road_full: str, key: str, display_road: str) -> GeoHit | None:
    tail = samcheok_tail(geocode_target(road_full))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if not m:
        return None
    area = collapse(m.group(1))
    for q in (f"강원 삼척시 {area}", f"강원특별자치도 삼척시 {area}"):
...
def main() -> None:
    ...
    ws = wb["종량제봉투 판매소"] if "종량제봉투 판매소" in wb.sheetnames else wb.active
    ...
    for row in ws.iter_rows(min_row=3, values_only=True):
        ...
        if not name or not addr_raw:
            continue
        has_trash = flag_o(row[2])
        has_special = flag_o(row[3])
        if not has_trash and not has_special:
            continue

        full_norm = normalize_samcheok_addr(addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "삼척" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"gangwon-samcheok-trash-{rid}",
                "name": name,
