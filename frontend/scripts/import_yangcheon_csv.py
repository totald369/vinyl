#!/usr/bin/env python3
"""
서울시 양천구 쓰레기 종량제봉투 판매소 현황 CSV(cp949) → stores.sample.json 병합.
- 열: 연번, 판매소명, 주소, …, 관할동명
- 주소가 서울로 시작하지 않으면 서울특별시 양천구 접두
- 카카오 주소/키워드 검색으로 위·경도
- 파일명 YYYYMMDD(예: _20231110.csv) → dataReferenceDate

사용:
  .env.local 의 KAKAO_REST_API_KEY
  python3 scripts/import_yangcheon_csv.py
  python3 scripts/import_yangcheon_csv.py /path/to/....csv
"""
...
CACHE_PATH = Path(__file__).resolve().parent / "geocode-cache-yangcheon.json"
...
def ref_date_from_path(path: Path) -> str | None:
    m = re.search(r"(\d{8})\.csv", path.name)
    if m:
        d = m.group(1)
...
def parse_csv(path: Path) -> list[dict]:
    ...
    for row in reader:
            if len(row) < 3:
                continue
            name = _str(row[1])
            frag = re.sub(r"\s+", " ", _str(row[2]))
            dong = _str(row[5]) if len(row) > 5 else ""
            if not name or not frag:
                continue
            if frag.startswith("서울"):
                addr = frag
            else:
                addr = f"서울특별시 양천구 {frag}"
            alts: list[str] = []
            if dong and (
                "일대" in frag
                or "번지" in frag
                ...
                alts.append(f"서울특별시 양천구 {dong} {name}")
                alts.append(f"양천구 {dong} {name}")
            out.append(
                {
                    "name": name,
                    "roadAddress": addr,
...
def discover_paths(argv: list[str]) -> list[Path]:
    ...
    for p in DOWNLOADS.glob("*.csv"):
        n = p.name
        if "양천" in n and ("봉투" in n or "종량제" in n or "판매" in n):
...
def main():
    ...
    if not paths:
        print("CSV 경로를 인자로 주거나, Downloads 에 양천 관련 CSV 를 두세요.")
        raise SystemExit(1)
