#!/usr/bin/env python3
"""
경상남도 거창군 종량제봉투·불연성마대 판매소 xlsx → stores.gyeongnam-geochang-trash.json

시트 `Sheet1`: 지자체명 | 상호명 | 주소 | 종량제봉투(O) | 특수마대(O) — 4행부터

  pip install openpyxl
  python3 scripts/import_gyeongnam_geochang_trash_from_xlsx.py \\
    --input ~/Downloads/정보공개청구\\(종량제봉투\\ 및\\ 특수마대\\ 판매처\\ 정보\\ 고액\\ 요청\\)\\(거창군\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-geochang-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-geochang-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "정보공개청구(종량제봉투 및 특수마대 판매처 정보 고액 요청)(거창군).xlsx"
REF_DATE = "2026-05-23"
CACHE_VERSION = "v1"
...
def in_geochang_bbox(lat: float, lng: float) -> bool:
    # 삼계·삼안·구산 일대(동경 128.91) 포함
    return 35.55 <= lat <= 35.95 and 127.55 <= lng <= 128.05


def geochang_in_text(blob: str) -> bool:
    return "거창" in (blob or "").replace(" ", "")
...
def format_display_addr(raw: str) -> str:
    ...
    if a.startswith("거창군"):
        return f"경상남도 {a}"
    if geochang_in_text(a) or re.search(r"(동|읍|면)\s", a):
        return f"경상남도 거창군 {a}"
    return a


def normalize_geochang_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    if not a.startswith("거창") and not a.startswith("경상"):
        a = f"거창군 {a}"
...
def geochang_tail(full: str) -> str:
    for prefix in ("경상남도 거창군 ", "경상남도 ", "경남 거창군 ", "거창군 "):
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    for base in road_format_variants(target):
        tail = geochang_tail(base)
        push(f"경남 거창군 {tail}")
        push(f"경상남도 거창군 {tail}")
        push(base)
        ...
        if road_only and road_only != tail:
            push(f"경남 거창군 {road_only}")
        m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
        if m:
            push(f"경남 거창군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_geochang_bbox(lat, lng):
        return None
    if not geochang_in_text(_doc_blob(d)):
        return None
...
def area_fallback(road_full: str, key: str, display_road: str) -> GeoHit | None:
    tail = geochang_tail(geocode_target(road_full))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if not m:
        return None
    area = collapse(m.group(1))
    for q in (f"경남 거창군 {area}", f"경상남도 거창군 {area}"):
...
def main() -> None:
    ...
    for row in ws.iter_rows(min_row=4, values_only=True):
        if not row or len(row) < 4:
            continue
        name = cell_str(row[1])
        addr_raw = cell_str(row[2])
        if not name or not addr_raw:
            continue
        has_trash = flag_o(row[3])
        has_special = flag_o(row[4]) if len(row) > 4 else False
        if not has_trash and not has_special:
            continue

        full_norm = normalize_geochang_addr(addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "거창" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"gyeongnam-geochang-trash-{rid}",
                "name": name,
