#!/usr/bin/env python3
"""
전북특별자치도 순창군 종량제봉투·불연성마대 판매소 xlsx → stores.jeonbuk-sunchang-trash.json

시트 `Sheet1` (헤더 1행, 데이터 2행~):
  연번(A) | 판매처 명칭(B) | 주소(C) | 연락처(D) | 종량제봉투(E) | 특수마대(F)
...
사용:
  cd frontend
  python3 scripts/import_jeonbuk_sunchang_trash_from_xlsx.py \\
    --input ~/Downloads/판매소\\ 관련\\ 정보공개청구(순창군).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonbuk-sunchang-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonbuk-sunchang-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "판매소 관련 정보공개청구(순창군).xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 2
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-sunchang"
...
def in_sunchang_bbox(lat: float, lng: float) -> bool:
    # 복흥면·동계면 등 서부 면은 경도 126.9대
    return 35.22 <= lat <= 35.58 and 126.88 <= lng <= 127.28


def sunchang_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "순창군" in t:
        return True
    return ("전북" in t or "전라북도" in t) and "순창" in t
...
def normalize_sunchang_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    if not a.startswith("전") and not a.startswith("순창"):
...
def normalize_phone(raw: object) -> str | None:
    ...
    digits = re.sub(r"\D", "", p)
    if len(digits) == 7:
        return f"063-{digits[:3]}-{digits[3:]}"
    if len(digits) >= 9:
        return p
    return p
...
def sunchang_tail(full: str) -> str:
    for prefix in (
        "전북특별자치도 순창군 ",
        "전북특별자치도 ",
        "전북 순창군 ",
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    if has_road:
        for base in road_format_variants(target):
            t = sunchang_tail(base)
            push(f"전북 순창군 {t}")
            push(f"전북특별자치도 순창군 {t}")
    elif is_likely_jibeon(tail):
        for tv in lot_query_variants(tail):
            push(f"전북 순창군 {tv}")
            push(f"전북특별자치도 순창군 {tv}")
    else:
        for base in road_format_variants(target):
            t = sunchang_tail(base)
            push(f"전북 순창군 {t}")
    ...
    if m:
        push(f"전북 순창군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_sunchang_bbox(lat, lng):
        return None
    if not sunchang_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = sunchang_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in SUNCHANG_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"전북 순창군 {area}", f"전북특별자치도 순창군 {area}"):
...
def iter_rows(path: Path) -> list[SunchangRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, SunchangRow] = {}

    def key_for(name: str, addr_raw: str) -> str:
        norm = normalize_sunchang_addr(addr_raw)
    ...
    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        ...
        k = key_for(name, addr)
        if k not in merged:
            merged[k] = SunchangRow(name, addr, phone, False, False)
        rec = merged[k]
        if has_trash:
            rec.has_trash = True
        if has_special:
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_sunchang_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "순창" not in display_road.replace(" ", ""):
            continue
        ...
        rec: dict = {
            "id": f"jeonbuk-sunchang-trash-{rid}",
            "name": row.name,
