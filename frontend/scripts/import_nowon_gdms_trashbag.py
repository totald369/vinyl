#!/usr/bin/env python3
"""
노원구 GDMS 물품판매소 지도 (main.asp) 에서 규격봉투(종량제) 판매처를 수집해
stores.sample.json 의 hasTrashBag 을 갱신합니다.

원본: POST http://119.192.103.73/gdms_maps/main.asp
      site=0221, dong=<동코드>, item=5003·5005·5010·5020·5050·5075 (규격봉투 리터별)

매칭: 인허가 상호와 현장 표기가 다를 수 있어 도로명/지번 문자열 겹침을 우선합니다.
미매칭 건은 카카오 지오코딩 후 120m 이내 최근접 노원구 매장에 hasTrashBag 을 켜고,
...
CACHE_GEO = Path(__file__).resolve().parent / "geocode-cache-nowon-gdms-trash.json"
...
def scrape_all() -> list[dict]:
    ...
    for dong in DONG_CODES:
        for item_code, _label in TRASH_ITEMS:
            try:
...
def main():
    ...
    print(
        f"hasTrashBag 갱신(기존 매칭) {matched}/{len(scraped)}건, "
