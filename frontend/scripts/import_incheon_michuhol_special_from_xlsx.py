#!/usr/bin/env python3
"""
인천광역시 미추홀구 불연성마대(특수마대) 판매소 xlsx → stores.incheon-michuhol-special.json

시트 `Sheet1` (헤더 6행, 데이터 7행~):
  연번(A) | 관할지역(B) | 판매소(C) | 주소(D)

  python3 scripts/import_incheon_michuhol_special_from_xlsx.py \\
    --input ~/Downloads/인천\\ 미추홀구\\ 불연성마대\\ 판매처\\ 목록.xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.incheon-michuhol-special.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-incheon-michuhol-special.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "인천 미추홀구 불연성마대 판매처 목록.xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 7
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-incheon-michuhol-special"
...
def in_michuhol_bbox(lat: float, lng: float) -> bool:
    # 관교·주안·문학 일대(경도 126.7대) 포함
    return 37.39 <= lat <= 37.50 and 126.60 <= lng <= 126.74


def michuhol_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("남동구", "연수구", "부평구", "서구", "계양구")):
        return False
    return "미추홀구" in t and ("인천" in t or "인천광역시" in t)
...
def dong_hint(addr_raw: str, district: str) -> str:
...
def michuhol_tail(full: str) -> str:
    for prefix in (
        "인천광역시 미추홀구 ",
        "인천광역시 ",
        "인천 미추홀구 ",
...
def geocode_query_variants(addr_raw: str, name: str, district: str) -> list[str]:
    clean_name = re.sub(r"\([^)]*\)", "", name).strip()
    norm = normalize_addr(addr_raw)
    hint = dong_hint(addr_raw, district)
    tail = michuhol_tail(norm)
    out: list[str] = []
    seen: set[str] = set()
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_michuhol_bbox(lat, lng):
        return None
    if not michuhol_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, district: str, key: str, display_road: str) -> GeoHit | None:
    hint = dong_hint(addr_raw, district)
    tail = michuhol_tail(normalize_addr(addr_raw))
    area = hint or ""
    if not area:
        m = re.search(r"([가-힣]+동)", tail)
        area = m.group(1) if m else district
    if not area:
        return None
    for q in (f"인천 미추홀구 {area}", f"인천광역시 미추홀구 {area}"):
...
def resolve_geocode(
    addr_raw: str, name: str, district: str, key: str, display_road: str
) -> GeoHit | None:
    for q in geocode_query_variants(addr_raw, name, district):
        for d in kakao_get(GEOCODE_URL, q, key):
            hit = parse_address_doc(d, key)
            if hit:
                return hit
        for d in kakao_get(KEYWORD_URL, q, key):
            ...
            if hit:
                return hit
    return area_fallback(addr_raw, district, key, display_road)
...
def iter_rows(path: Path) -> list[MichuholRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    out: list[MichuholRow] = []
    current_district = ""

    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        district = cell_str(ws.cell(r, COL_DISTRICT).value)
        if district:
            current_district = district
        name = clean_store_name(cell_str(ws.cell(r, COL_NAME).value))
        addr_raw = cell_str(ws.cell(r, COL_ADDR).value)
        if not name or not addr_raw or name in ("판매소",):
            continue
        out.append(MichuholRow(name=name, addr_raw=addr_raw, district=current_district))
...
def main() -> None:
    ...
    for row in rows:
        ...
        if not display_road or "미추홀" not in display_road.replace(" ", ""):
            continue
        ...
        if hit is None and allow:
            hit = resolve_geocode(row.addr_raw, row.name, row.district, key, display_road)
            if hit is None:
                misses += 1
        ...
        out.append(
            {
                "id": f"incheon-michuhol-special-{rid}",
                "name": row.name,
