#!/usr/bin/env python3
"""
서울시 구로구 종량제봉투 판매소 CSV → stores.sample.json 병합.
- 열: 연번, 행정동, 상점명, 주소, 데이터기준일자(YYYY-MM-DD, 선택)
- 주소가 서울로 시작하지 않으면 서울특별시 구로구 접두
- 카카오 주소/키워드 검색으로 위·경도
- 행의 데이터기준일자가 있으면 우선, 없으면 파일명 YYYYMMDD

사용:
  .env.local 의 KAKAO_REST_API_KEY
  python3 scripts/import_guro_csv.py
  python3 scripts/import_guro_csv.py /path/to/....csv
"""
...
CACHE_PATH = Path(__file__).resolve().parent / "geocode-cache-guro.json"
...
def parse_csv(path: Path) -> list[dict]:
    ...
    for row in reader:
        ...
        if dong and (
            ...
            alts.append(f"서울특별시 구로구 {dong} {name}")
            alts.append(f"구로구 {dong} {name}")
        out.append(
            {
                "name": name,
                "roadAddress": addr,
...
def discover_paths(argv: list[str]) -> list[Path]:
    ...
    for p in DOWNLOADS.glob("*.csv"):
        n = p.name
        if "구로" in n and ("봉투" in n or "종량제" in n or "판매" in n):
...
def main():
    ...
    if not paths:
        print("CSV 경로를 인자로 주거나, Downloads 에 구로 관련 CSV 를 두세요.")
        raise SystemExit(1)
