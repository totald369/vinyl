#!/usr/bin/env python3
"""
서울특별시 마포구 특수규격봉투(불연성마대) 판매소 xlsx → stores.seoul-mapo-special.json

시트 `Sheet1` (헤더 2행, 데이터 3행~):
  행정동(A) | 상호(B) | 전화번호(C) | 소재지(D) | 공급대행업체(E)
...
사용:
  cd frontend
  python3 scripts/import_seoul_mapo_special_from_xlsx.py \\
    --input ~/Downloads/마포구_특수규격봉투\\(마대\\)\\ 판매소\\ 목록\\(2026.5\\)_불연성폐기물\\ 처리용.xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.seoul-mapo-special.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-seoul-mapo-special.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "마포구_특수규격봉투(마대) 판매소 목록(2026.5)_불연성폐기물 처리용.xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 3
REF_DATE = "2026-05-01"
CACHE_VERSION = "v1-seoul-mapo-special"
...
def ref_date_from_path(p: Path) -> str:
    m = re.search(r"\(20(\d{2})\.(\d{1,2})\)", p.name)
    if m:
...
def in_mapo_bbox(lat: float, lng: float) -> bool:
    return 37.53 <= lat <= 37.59 and 126.88 <= lng <= 126.96


def mapo_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if "마포구" in t:
        return True
    return ("서울" in t or "서울특별시" in t or "서울시" in t) and "마포" in t
...
def normalize_addr(addr_raw: str, dong: str = "") -> str:
    ...
    compact = a.replace(" ", "")
    if dong_clean and dong_clean not in compact and ROAD_RE.search(a):
        a = f"{dong_clean} {a}"
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_mapo_bbox(lat, lng):
        return None
    if not mapo_in_text(_doc_blob(d)):
        return None
...
def area_fallback(dong: str, key: str, display_road: str) -> GeoHit | None:
    area = collapse(dong)
    if not area:
        return None
    for q in (f"서울 마포구 {area}", f"서울특별시 마포구 {area}"):
...
def resolve_geocode(addr_raw: str, name: str, key: str, dong: str, display_road: str) -> GeoHit | None:
    ...
    for q in geocode_query_variants(addr_raw, name, dong):
        ...
        if not has_road:
            for d in kakao_get(GEOCODE_URL, q, key):
                ...
                if hit:
                    return hit
    return area_fallback(dong, key, display_road)
...
def iter_rows(path: Path) -> list[MapoRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, MapoRow] = {}
    last_dong = ""

    for r in range(DATA_START_ROW, (ws.max_row or 0) + 1):
        dong = cell_str(ws.cell(r, COL_DONG).value)
        if dong:
            last_dong = dong
...
def main() -> None:
    ...
    for row in rows:
        ...
        if hit is None and allow:
            hit = resolve_geocode(row.addr_raw, row.name, key, row.dong, display_road)
            if hit is None:
                misses += 1
