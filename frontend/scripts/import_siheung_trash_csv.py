#!/usr/bin/env python3
"""
시흥시 종량제봉투 판매소 CSV -> stores.gyeonggi-siheung-trash.json
...
DOWNLOAD_CSV = (
    Path.home() / "Downloads" / "경기도_시흥시_종량제봉투판매소현황_20251112.csv"
)
FRONTEND = Path(__file__).resolve().parent.parent
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeonggi-siheung-trash.json"
CACHE_PATH = Path(__file__).resolve().parent / "geocode-cache-siheung-trash.json"
...
def main() -> None:
    ...
    for r in reader:
        ...
        raw_addr = (r.get("도로명 주소") or "").strip()
    ...
    for i, r in enumerate(rows, start=1):
        ...
        if ck in cache:
            ...
            for base in addr_variants(r["roadAddress"]):
                for q in (base, f"{r['name']} {base}", f"시흥시 {r['name']}"):
            ...
            if lat is None or lng is None:
                continue
            if not (37.32 <= lat <= 37.50 and 126.70 <= lng <= 126.95):
                continue
            cache[ck] = [lat, lng]
            if i % 40 == 0:
                save_cache(cache)
