#!/usr/bin/env python3
"""
경상남도 밀양시 종량제봉투·불연성마대 판매소 xlsx → stores.gyeongnam-miryang-trash.json

시트 `밀양시`: 판매소명 | 도로명 주소 | 종량제봉투(O) | 불연성 마대(O) — 5행부터

  pip install openpyxl
  python3 scripts/import_gyeongnam_miryang_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투 판매소 현황(밀양시).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-miryang-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-miryang-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매소 현황(밀양시).xlsx"
REF_DATE = "2026-05-23"
CACHE_VERSION = "v1"
...
def in_miryang_bbox(lat: float, lng: float) -> bool:
    # 삼계·삼안·구산 일대(동경 128.91) 포함
    return 35.38 <= lat <= 35.58 and 128.58 <= lng <= 128.88


def miryang_in_text(blob: str) -> bool:
    return "밀양" in (blob or "").replace(" ", "")
...
def format_display_addr(raw: str) -> str:
    ...
    if a.startswith("밀양시"):
        return f"경상남도 {a}"
    if miryang_in_text(a) or re.search(r"(동|읍|면)\s", a):
        return f"경상남도 밀양시 {a}"
    return a


def normalize_miryang_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    if not a.startswith("밀양") and not a.startswith("경상"):
        a = f"밀양시 {a}"
...
def miryang_tail(full: str) -> str:
    for prefix in ("경상남도 밀양시 ", "경상남도 ", "경남 밀양시 ", "밀양시 "):
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    for base in road_format_variants(target):
        tail = miryang_tail(base)
        push(f"경남 밀양시 {tail}")
        push(f"경상남도 밀양시 {tail}")
        push(base)
        ...
        if road_only and road_only != tail:
            push(f"경남 밀양시 {road_only}")
        m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
        if m:
            push(f"경남 밀양시 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_miryang_bbox(lat, lng):
        return None
    if not miryang_in_text(_doc_blob(d)):
        return None
...
def area_fallback(road_full: str, key: str, display_road: str) -> GeoHit | None:
    tail = miryang_tail(geocode_target(road_full))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if not m:
        return None
    area = collapse(m.group(1))
    for q in (f"경남 밀양시 {area}", f"경상남도 밀양시 {area}"):
...
def main() -> None:
    ...
    for row in ws.iter_rows(min_row=5, values_only=True):
        ...
        full_norm = normalize_miryang_addr(addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "밀양" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"gyeongnam-miryang-trash-{rid}",
                "name": name,
