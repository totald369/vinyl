#!/usr/bin/env python3
"""
경상남도 함양군 종량제봉투 판매소 xlsx → stores.gyeongnam-hamyang-trash.json
...
사용:
  cd frontend
  python3 scripts/import_gyeongnam_hamyang_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 판매소\\ 현황\\(함양군\\).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.gyeongnam-hamyang-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-gyeongnam-hamyang-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매소 현황(함양군).xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 4
REF_DATE = "2026-06-09"
CACHE_VERSION = "v1-hamyang"
...
def in_hamyang_bbox(lat: float, lng: float) -> bool:
    return 35.36 <= lat <= 35.72 and 127.62 <= lng <= 127.84


def hamyang_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "함양군" in t:
        return True
    return ("경남" in t or "경상남도" in t) and "함양" in t
...
def normalize_hamyang_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = re.sub(r"\s*\([^)]*\)\s*", " ", a)
    ...
    if not a.startswith("경") and not a.startswith("함양"):
        a = f"함양군 {a}"
...
def geocode_query_variants(road_full: str, name: str) -> list[str]:
    ...
    if m:
        push(f"경남 함양군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_hamyang_bbox(lat, lng):
        return None
    if not hamyang_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = hamyang_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in HAMYANG_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"경남 함양군 {area}", f"경상남도 함양군 {area}"):
...
def iter_rows(path: Path) -> list[HamyangRow]:
    wb = load_workbook(path, data_only=True)
    ws = wb[SHEET_NAME] if SHEET_NAME in wb.sheetnames else wb.active
    merged: dict[str, HamyangRow] = {}

    def key_for(name: str, addr_raw: str) -> str:
        norm = normalize_hamyang_addr(addr_raw)
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_hamyang_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "함양" not in display_road.replace(" ", ""):
            continue
        ...
        out.append(
            {
                "id": f"gyeongnam-hamyang-trash-{rid}",
                "name": row.name,
