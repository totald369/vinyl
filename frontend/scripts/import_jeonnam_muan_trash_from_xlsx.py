#!/usr/bin/env python3
"""
전라남도 무안군 종량제봉투·불연성마대 판매소 xlsx → stores.jeonnam-muan-trash.json

시트 `Sheet1` (헤더 3~4행, 데이터 5행~):
  연번(B) | 상호(C) | 소재지(D) | 연락처(E) | 매립용(불연성)봉투(F) | 음식물용기(G)

매핑:
  - 시트 등재(종량제봉투 판매소 현황) -> hasTrashBag
  - F열 '○' -> hasSpecialBag (불연성마대)
  - 음식물용기(G)는 제외

사용:
  cd frontend
  python3 scripts/import_jeonnam_muan_trash_from_xlsx.py \\
    --input ~/Downloads/종량제봉투\\ 판매소\\ 현황(무안군).xlsx
...
OUT_JSON = FRONTEND / "public" / "data" / "stores.jeonnam-muan-trash.json"
CACHE_PATH = SCRIPT_DIR / "geocode-cache-jeonnam-muan-trash.json"
DL = Path.home() / "Downloads"
DEFAULT_INPUT = DL / "종량제봉투 판매소 현황(무안군).xlsx"
SHEET_NAME = "Sheet1"
DATA_START_ROW = 5
REF_DATE = "2026-01-01"
CACHE_VERSION = "v1-muan"
...
def in_muan_bbox(lat: float, lng: float) -> bool:
    return 34.78 <= lat <= 35.18 and 126.22 <= lng <= 126.78


def muan_in_text(blob: str) -> bool:
    t = (blob or "").replace(" ", "")
    if any(x in t for x in ("서울", "대구", "부산", "인천", "대전", "울산")):
        return False
    if "무안군" in t:
        return True
    return ("전라남도" in t or "전남" in t) and "무안" in t
...
def normalize_muan_addr(addr_raw: str) -> str:
    a = collapse(addr_raw)
    if not a:
        return ""
    a = a.replace("삼햡읍", "삼향읍")
    ...
    a = re.sub(r"무안군(?=[가-힣])", "무안군 ", a)
    a = re.sub(r"^전남\s+", "전라남도 ", a)

    if not re.search(r"(?:읍|면|군|시)", a):
        a = f"무안읍 {a}"

    if not a.startswith("전라") and not a.startswith("전남") and not a.startswith("무안"):
        if re.match(r"^\S+?(?:읍|면)\s", a):
            a = f"무안군 {a}"
        else:
            a = f"무안군 무안읍 {a}"
    ...
    if is_likely_jibeon(tail):
        ...
        for base in road_format_variants(target):
            ...
            if road_only and road_only != t:
                push(f"전남 무안군 {road_only}")

    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    if m:
        push(f"전남 무안군 {collapse(m.group(1))}")
...
def parse_address_doc(d: dict, kakao_key: str) -> GeoHit | None:
    ...
    if lat is None or lng is None or not in_muan_bbox(lat, lng):
        return None
    if not muan_in_text(_doc_blob(d)):
        return None
...
def area_fallback(addr_raw: str, key: str, display_road: str) -> GeoHit | None:
    tail = muan_tail(geocode_target(addr_raw))
    m = re.match(r"^(.+?(?:동|읍|면))", tail.replace(" ", ""))
    area = collapse(m.group(1)) if m else ""
    if not area:
        for candidate in MUAN_AREAS:
            if candidate in tail:
                area = candidate
                break
    if not area:
        return None
    for q in (f"전남 무안군 {area}", f"전라남도 무안군 {area}"):
...
def iter_rows(path: Path) -> list[MuanRow]:
...
def main() -> None:
    ...
    for row in rows:
        full_norm = normalize_muan_addr(row.addr_raw)
        display_road = geocode_target(full_norm)
        if not display_road or "무안" not in display_road.replace(" ", ""):
            continue
