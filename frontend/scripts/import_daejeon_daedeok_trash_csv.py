#!/usr/bin/env python3
"""
대전광역시 대덕구 종량제봉투 판매소 CSV -> stores.daejeon-daedeok-trash.json

CSV 컬럼 (예시):
- 상호명, 소재지지번주소
- 종량제봉투취급여부, 대형폐기물스티커취급여부, 특수규격봉투취급여부 (Y/N)
  → 특수규격봉투 = 앱의 hasSpecialBag (불연성 마대 등)
- 데이터기준일자
"""
...
DOWNLOAD_CSV = (
    Path.home()
    / "Downloads"
    / "대전광역시 대덕구_종량제봉투판매소_20250701.csv"
)
FRONTEND = Path(__file__).resolve().parent.parent
OUT_JSON = FRONTEND / "public" / "data" / "stores.daejeon-daedeok-trash.json"
CACHE_PATH = Path(__file__).resolve().parent / "geocode-cache-daejeon-daedeok-trash.json"
...
def pick_ref_date(r: dict[str, str], default: str) -> str:
    s = (r.get("데이터기준일자") or "").strip()
    m = DATE_RE.match(s)
    if m:
        return m.group(1)
    return default
...
def normalize_addr(addr: str) -> str:
    ...
    if not a:
        return a
    if a.startswith("대전광역시 대덕구"):
        return a
    if a.startswith("대덕구") and not a.startswith("대전"):
        return f"대전광역시 {a}"
    if a.startswith("대전 대덕구"):
        return "대전광역시 " + a[len("대전") :].strip()
    return f"대전광역시 대덕구 {a}"
...
def addr_variants(addr: str) -> list[str]:
    ...
    add(re.sub(r"(\d+번길)(\d+)$", r"\1 \2", head).strip())
...
def in_daejeon_daedeok_bbox(lat: float, lng: float) -> bool:
    return 36.34 <= lat <= 36.54 and 127.38 <= lng <= 127.53
...
def main() -> None:
    ...
    default_ref = "2025-07-01"

    dedup: dict[str, dict] = {}
    for r in reader:
        name = (r.get("상호명") or "").strip()
        raw_addr = (r.get("소재지지번주소") or "").strip()
        ...
        if k not in dedup:
            dedup[k] = {
                ...
                "roadAddress": addr,
                "hasTrashBag": yn(r.get("종량제봉투취급여부")),
                "hasLargeWasteSticker": yn(r.get("대형폐기물스티커취급여부")),
                # 특수규격봉투 → 불연성 마대 등, 앱의 hasSpecialBag
                "hasSpecialBag": yn(r.get("특수규격봉투취급여부")),
                "dataReferenceDate": pick_ref_date(r, default_ref),
            }
    ...
    for i, r in enumerate(rows, start=1):
        sid = f"daejeon-daedeok-trash-{i:04d}"
        ...
        if ck in cache:
            ...
            if lat is None or lng is None:
                continue
            if not in_daejeon_daedeok_bbox(lat, lng):
                continue
            cache[ck] = [lat, lng]
            if i % 40 == 0:
                save_cache(cache)
        ...
        if not in_daejeon_daedeok_bbox(float(lat), float(lng)):  # type: ignore[arg-type]
            continue

        ref = r["dataReferenceDate"]
        rec = {
            "id": sid,
            "name": r["name"],
            ...
            "dataReferenceDate": existing_meta.get(sid, {}).get(
                "dataReferenceDate", ref or today
            ),
        }
